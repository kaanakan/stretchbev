...
from stretchbev.config import get_cfg
from stretchbev.losses import SpatialRegressionLoss, SegmentationLoss
from stretchbev.metrics import IntersectionOverUnion, PanopticMetric
from stretchbev.models import model_utils
from stretchbev.models.stretchbev import StretchBEV
from stretchbev.utils.geometry import cumulative_warp_features_reverse
from stretchbev.utils.instance import predict_instance_segmentation_and_trajectories
from stretchbev.utils.visualisation import visualise_output
...
class TrainingModule(pl.LightningModule):
    ...
    def shared_step(self, batch, is_train):
        ...
        loss['state_reconstruction'] = reconstruction_factor * F.mse_loss(all_output['generated_srvp_x'],
                                                                          all_output['lss_outs'])
        ...
        q_z, p_z = model_utils.make_normal_from_raw_params(
            all_output['q_z_params']), model_utils.make_normal_from_raw_params(all_output['p_z_params'])
    ...
    def prepare_future_labels(self, batch, receptive_field=1):
        ...
        future_distribution_inputs = F.adaptive_max_pool2d(future_distribution_inputs.reshape(b, t * n, h, w),
                                                           50).reshape(b, t, n, 50, 50)
